...
func RegisterAPIServices(path string, r *mux.Router) {
	...
	api.HandleFunc("/edges", NewCommand2(EdgesFormatter, "dump", "edges").Handler()).Methods("GET")
	...
	api.HandleFunc("/parameter", NewCommand3(GetFormatter, "name", "get").Handler()).Methods("GET")
	...
	api.HandleFunc("/auth", auth.Info).Methods("GET")
